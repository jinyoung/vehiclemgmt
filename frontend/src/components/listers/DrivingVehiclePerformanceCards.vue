    ...
    <div>
        ...
        >
            <v-row>
                <v-list-item class="d-flex" style="background-color: white;">
                    ...
                    <div class="secondary-text-color" style="margin-left:30px;">운행 실적을 저장하고 조회하는 기능을 제공합니다</div>
                </v-list-item>
            </v-row>
        </v-card>
        <v-col style="margin-bottom:40px;">
            <div class="text-center">
                ...
                >
                    <DrivingVehiclePerformance :offline="offline" class="video-card" :isNew="true" :editMode="true" v-model="newValue" 
        ...
        <v-row>
            <DrivingVehiclePerformance :offline="offline" class="video-card" v-for="(value, index) in values" v-model="values[index]" v-bind:key="index" @delete="remove"/>
        </v-row>
    </div>
</template>
...
<script>
    ...
    import DrivingVehiclePerformance from './../DrivingVehiclePerformance.vue';

    export default {
        name: 'DrivingVehiclePerformanceManager',
        components: {
            DrivingVehiclePerformance,
        },
        props: {
            offline: Boolean
        },
