                ...
                <v-list-item v-for="(item, idx) in list" :key="idx">
                    <template v-slot:default="{ active }">
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                AccumulatedDistanceBefore :  {{item.accumulatedDistanceBefore }}
                            </v-list-item-subtitle>
                            <v-list-item-subtitle>
...
<script>
    ...
    export default {
        name: 'DrivingVehiclePerformancePicker',
        props: {
        ...
        async created() {
            ...
            if(temp.data) {
                me.list = temp.data._embedded.vehiclePerformances;
            }

            if(me.value && typeof me.value == "object" && Object.values(me.value)[0]) {
                var id = Object.values(me.value)[0];
                var tmpValue = await axios.get(axios.fixUrl('/vehiclePerformances/' + id))
                if(tmpValue.data) {
